...
with st.sidebar:
    ...
    # 质心位置
    mu_X1 = st.slider('mu_X1', min_value=-4.0, max_value=4.0, value=0.0, step=0.1)
    mu_X2 = st.slider('mu_X2', min_value=-4.0, max_value=4.0, value=0.0, step=0.1)
    # 标准差
    sigma_X1 = st.slider('sigma_X1', min_value=0.5, max_value=3.0, value=1.0, step=0.1)
    sigma_X2 = st.slider('sigma_X2', min_value=0.5, max_value=3.0, value=1.0, step=0.1)
    # 相关性系数
...
plt.axvline(x=mu_X1, color='k', linestyle='--'); plt.axhline(y=mu_X2, color='k', linestyle='--')
